...
const ExploreTrek = () => {
  ...
  const fetchForecast = async (lat, lon, trekId) => {
    ...
    try {
      ...
      console.error("Forecast fetch failed:", err.response?.data || err.message);
    }
  };
  ...
  useEffect(() => {
    ...
    const fetchAllTreksWeather = async () => {
      ...
      const updated = {};
      results.forEach((res) => { updated[res.id] = res.weather; });
      ...
      <div className="explore-wrapper">
        ...
        <div className="trek-grid">
          {filteredTreks.map((trek) => {
            ...
            const fallbackText = trek.user?.name?.charAt(0)?.toUpperCase() || "U";
            ...
            return (
              ...
              >
                ...
                <div className="card-header">
                  {/* Profile pic + name → navigate to user profile */}
                  <div
                    className="user-info"
                    onClick={(e) => {
                      e.stopPropagation();
                    ...
                    style={{ gap: "12px", alignItems: "center", cursor: "pointer" }}
                  >
                    <div
                    ...
                    <span style={{ fontSize: "17px", fontWeight: 600, color: "#111827" }}>
                      {trek.user?.name}
                    </span>
                  </div>
                ...
                >
                  ...
                  >
                    <FaHeart
                      style={{
                        color: trek.likes?.some((u) => u._id === userId) ? "red" : "gray",
                        fontSize: "18px",
                      }}
                    />
        ...
        <div className="forecast-modal-overlay" onClick={closeForecast}>
          <div className="forecast-modal" onClick={(e) => e.stopPropagation()}>
            <div className="forecast-header">
              ...
              <button className="close-btn" onClick={closeForecast}>×</button>
            </div>
              ...
              {selectedTrek.comments.map((c) => (
                <div key={c._id} className="comment-item">
                  <div className="comment-header">
                    <img src={`http://localhost:5000${c.user.profilePic}`} alt="" />
