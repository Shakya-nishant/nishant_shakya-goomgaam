...
router.get("/me", protect, async (req, res) => {
  try {
    ...
    treks.forEach((trek) => {
      rewardPoints += 20; // base for posting trek
      rewardPoints += (trek.photos?.length || 0) * 5; // points for photos
      if (trek.routePoints?.length || trek.difficulty || trek.travelTips) rewardPoints += 10;
      ...
      if (trek.routePoints?.length > 1) {
        for (let i = 0; i < trek.routePoints.length - 1; i++) {
          ...
          const distanceKm = getDistanceFromLatLonInKm(from.lat, from.lng, to.lat, to.lng);
    ...
    for (const user of allUsers) {
      ...
      userTreks.forEach((trek) => {
        ...
        if (trek.routePoints?.length || trek.difficulty || trek.travelTips) userPoints += 10;
    ...
    const rank = leaderboard.findIndex((u) => u.userId === userId.toString()) + 1;
